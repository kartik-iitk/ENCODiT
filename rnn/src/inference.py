...
parser.add_argument('--ckpt', default='wl256_11061039/best_model_381.pt', help="path load the trained network")
parser.add_argument('--gpu', default='0', type=str, help='id(s) for CUDA_VISIBLE_DEVICES')
parser.add_argument('--trials', type=int, default=1, help='no. of trials for taking average for the final results')
parser.add_argument('--wl', type=int, default=256, help='window length')
...
parser.add_argument('--cal_root_dir', type=str, default='./',
                    help='calibration data directory')
parser.add_argument('--in_test_root_dir', type=str, default='./',
                    help='test data directory')
parser.add_argument('--out_test_root_dir', type=str, default='./',
                    help='test data directory')
parser.add_argument('--transformation_list', '--names-list', nargs='+', default=["low_pass", "high_pass", "identity", "high_low"])
...
def calc_cal_ce_loss(opt, model, criterion, device,
                     cal_dataloader):  # for calibration datapoint, we want one randomly sampled window for 1 datapoint
    torch.set_grad_enabled(False)
    model.eval()
    ...
    for iter in range(0,
                      opt.n):  # n iterations with random sampling of windows and transformations on calibration datapoints
        print("n: ", iter + 1)
        ce_loss = []
...
def checkOOD(n=opt.n):
    ...
    cal_dataset = GAIT(root_dir=opt.cal_root_dir, win_len=opt.wl, train=False, cal=True, in_dist_test=False,
    ...
    cal_set_ce_loss_all_iter = calc_cal_ce_loss(opt, model=net, criterion=criterion, device=device,
                                                cal_dataloader=cal_dataloader)  # cal_set_ce_loss_all_iter = 2D vector with opt.n verctors, each vector contains loss for all calibration datapoints
    ...
    in_test_dataset = GAIT(root_dir=opt.in_test_root_dir, win_len=opt.wl, train=False, cal=False, in_dist_test=True,
                           transformation_list=opt.transformation_list)
    ...
    out_test_dataset = GAIT(root_dir=opt.out_test_root_dir, win_len=opt.wl, train=False, cal=False, in_dist_test=False,
                            transformation_list=opt.transformation_list, disease_type='')
    ...
    for iter in range(0, opt.n):  # n iterations
        ...
        for test_idx in range(0, len(out_test_ce_loss)):  # iter over all test datapoints
            ...
            for window_idx in range(0, len(out_test_ce_loss[test_idx])):  # iteration over windows of the test datapoint
                out_p_values.append(
                    calc_p_value(out_test_ce_loss[test_idx][window_idx], cal_set_ce_loss_all_iter[iter]))
            out_p_values_all_traces.append(np.array(out_p_values))
        np.savez("{}/out_p_values_iter{}.npz".format(opt.save_dir, iter + 1),
...
def calc_fisher_batch(p_values, eval_n):  # p_values is 3D
    output = [[None] * len(window) for window in p_values[
        0]]  # output is a 2D list for each datapoint, no of datapoints X number of windows in each datapoint
...
def eval_detection_fisher(eval_n):
    ...
    in_fisher_values = calc_fisher_batch(in_p,
                                         eval_n)  # a 2D fisher value output for each window in each iD test datapoint
    out_fisher_values = calc_fisher_batch(out_p,
                                          eval_n)  # a 2D fisher value output for each window in each OOD test datapoint
    # pdb.set_trace()
...
if __name__ == "__main__":
    ...
    for trial in range(opt.trials):
        ...
        for i in range(opt.n):
            ...
            #print("ai",in_fisher_values_per_win.shape, out_fisher_values_per_win.shape)
    ...
    print("AUROC for CODiT(n=100) on {} as OOD data with window length {} is {}".format('', opt.wl,
                                                                                        np.mean(auroc_all_trials, 0)[
                                                                                            -1]))
