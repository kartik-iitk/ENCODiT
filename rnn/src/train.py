...
def parse_args():
    ...
    parser.add_argument('--log', type=str, default='./', help='log directory')
    ...
    parser.add_argument('--train_root_dir', type=str, default='gait-in-neurodegenerative-disease-database-1.0.0',
                        help='training data directory')
    parser.add_argument('--cal_root_dir', type=str, default='gait-in-neurodegenerative-disease-database-1.0.0',
                        help='calibration data directory')
    parser.add_argument('--test_root_dir', type=str, default='gait-in-neurodegenerative-disease-database-1.0.0',
                        help='test data directory')
    parser.add_argument('--transformation_list', '--names-list', nargs='+',
                        default=["low_pass", "high_pass", "identity", "high_low"]) #high_pass low_high high_low identity
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':  ########### Train #############
        ...
        train_dataset = GAIT(root_dir=args.train_root_dir, win_len=args.wl, train=True, cal=False,
                             transformation_list=args.transformation_list)

        val_dataset = GAIT(root_dir=args.cal_root_dir, win_len=args.wl, train=False, cal=True, in_dist_test=False,
                           transformation_list=args.transformation_list)
        ...
        test_dataset = GAIT(root_dir=args.test_root_dir, win_len=args.wl, train=False, cal=False, in_dist_test=False,
                            transformation_list=args.transformation_list)
